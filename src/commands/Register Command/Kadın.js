...
let erkekemdes = new EmbedBuilder()
  ...
  .setDescription(`${uye.toString()} **KADIN** olarak kayıt edildi! ${green} `)
    ...
    if(ayar.chatChannel && client.channels.cache.has(ayar.chatChannel)) client.channels.cache.get(ayar.chatChannel).send({ content:`> ${Hello} Aramıza ${uye} Güzel Bayan katıldı onu Merhaba ile karşılayın.`}).then((e) => setTimeout(() => { e.delete(); }, 15000));
