...
const { red, green, jail } = require("../../../settings/configs/emojis.json")
  ...
  //    .setAuthor({ name: uye.displayName, iconURL: uye.user.displayAvatarURL({ dynamic: true }) })
   ...
   //   .setAuthor({ name: uye.displayName, iconURL: uye.user.displayAvatarURL({ dynamic: true }) })
    ...
    if (message.member.roles.highest.position <= member.roles.highest.position) return message.reply({ embeds: [new EmbedBuilder()
     // .setAuthor({ name: uye.displayName, iconURL: uye.user.displayAvatarURL({ dynamic: true }) })
      .setThumbnail()
      .setDescription(`${red} Kendinle aynı yetkide ya da daha yetkili olan birini jailleyemezsin!`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    ...
    let papaz31 = member.user.username.slice(0, 15)
      yazı.push(`${papaz31}...`)  
    } else {
