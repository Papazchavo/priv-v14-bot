    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !kanallar.includes(message.channel.name)) return message.reply({ content: `${kanallar.map(x => `${client.channels.cache.find(chan => chan.name == x)}`)} kanallarında kullanabilirsiniz.`}).then((e) => setTimeout(() => { e.delete(); }, 10000)); 
        ...
        const data = await bannedTag.findOne({ guildID: allah.GuildID });
            ...
            if (args[0] == "ekle") {
                ...
                if (!data) {
                    ...
                    let üyeler = message.guild.members.cache.filter(x => {
                        return x.user.globalName.includes(args[1])
                    })
            ...
            if (args[0] == "liste" && args[1] == "üye") {
                ...
                let üyeler = message.guild.members.cache.filter(x => {
                    return x.user.globalName.includes(args[2])
                }).map(x => "<@" + x.id + "> - (`" + x.id + "`)")
                let üyelerk = message.guild.members.cache.filter(x => {
                    return x.user.globalName.includes(args[2])
                }).map(x => "" + x.user.globalName + " - (`" + x.id + "`)")
            ...
            if (args[0] == "kontrol") {
                ...
                data.taglar.forEach(x => {
                    let üye = message.guild.members.cache.filter(mems => {
                        return mems.user.globalName.includes(x) && !mems.roles.cache.has(conf.yasaklıRole)
                    }).map(x => x.id)
