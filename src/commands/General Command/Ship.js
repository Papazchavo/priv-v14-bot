...
const roller = require("../../../settings/configs/sunucuayar.json")
const papazcim = require("../../../config.json")
const { red, Hello } = require("../../../settings/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args) => {
    if(!message.channel.name.includes("ship-chat")) return message.reply(`Bu Komutu Sadece ${message.guild.channels.cache.filter(x => x.name.includes("ship-chat")).map(x => x).join(", ")} kanallarında kullanabilirsiniz.`).then(x => {
        message.react(red)
  ...
  let emoji = client.emojis.cache.find(x => x.name === "Hello")  // sunucunda kalps adında emoji olursa gösterir
...
const özelKullanıcılar = ["469419605895544832"];// buraya hangi ID girersen full çıkar ikisine attığında ship

let love;
let emoticon;

if (özelKullanıcılar.includes(member)) {
...
if (emoti <= 44 && emoti >= 23) {
    ...
    const imagePath = path.resolve(__dirname, "../../../settings/Assets/ship.png");
