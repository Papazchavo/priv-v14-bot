...
const { green, red,info } = require("../../../settings/configs/emojis.json")
...
run: async (client, message, args, embed) => {
    ...
    const row2 = new Discord.ActionRowBuilder().addComponents(
        new Discord.ButtonBuilder().setCustomId("allres").setLabel("Tüm Botları Yeniden Başlat").setEmoji(info).setStyle(Discord.ButtonStyle.Danger),
    );
    ...
    collector.on("collect", async (menu) => {
        ...
        if (menu.customId == "allres") {
            ...
            if (mesaj) mesaj.delete().catch(() => { });
            children.exec(`${allah.GuildName}_Moderation`);
        }
    });
