...
const bannedCmd = require("../../settings/schemas/bannedCmd")
setInterval(() => {
...
if (veri.kullanici.includes(message.member.id)) return message.reply({embeds: [embed.setDescription(`${message.member} Komut kullanımınız yasaklanmış.`)]})
  ...
  if (cmd) {
    ...
    return komutLog.send({ content: `tarafından ${message.channel} kanalında \`${prefix}${commandName}\` komutu kullanıldı.`}).catch(() => {});
  }
};
