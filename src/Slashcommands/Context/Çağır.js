const { PermissionsBitField, ButtonStyle, ComponentType, ContextMenuCommandBuilder, hyperlink, ApplicationCommandType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, IntegrationApplication } = require("discord.js");
...
if (interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
    ...
    interaction.reply({ embeds: [new EmbedBuilder().setThumbnail(interaction.user.avatarURL({ dynamic: true, size: 2048 })).setDescription(`${interaction.user}, ${member} kişisini yanınıza taşıdınız.`)], ephemeral: true });
